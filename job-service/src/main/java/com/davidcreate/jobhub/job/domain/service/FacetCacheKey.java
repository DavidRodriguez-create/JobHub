...
/**
 * Cache key for the facets response (ADR 0020), derived from the FILTER set
 * only: {@code page}, {@code size}, and {@code sort} are deliberately excluded,
 * identical rationale to {@link CountCacheKey} (ADR 0018). Facets are a property
 * of which postings match, not of how the matching page is sliced or ordered.
 * Two {@link JobSearchQuery} instances with identical filter fields always
 * produce equal keys regardless of pagination/sort (AC-332-7); differing in any
 * single filter field always produces unequal keys (AC-332-12).
 */
public record FacetCacheKey(
        String keyword,
    ...
    public static FacetCacheKey from(JobSearchQuery query) {
        return new FacetCacheKey(
                query.getKeyword(),
                query.getLocations(),
                query.getLanguages(),
                query.getCompanies(),
