package com.davidcreate.jobhub.crawler.adapter.in.rest.filter;
 ...
 * (ADR 0008, ADR 0033). This is service-to-service authentication, not user identity: it
 * does not interact with a JWT stack at all (crawler-service has none). Requests to
 * non-internal paths pass through untouched.
 */
@Provider
@PreMatching
    ...
    public void filter(ContainerRequestContext requestContext) {
        ...
        if (providedKey == null || !providedKey.equals(expectedKey)) {
            requestContext.abortWith(Response.status(Response.Status.UNAUTHORIZED)
                    ...
                    .entity(new ErrorResponse("Unauthorized", "Missing or invalid X-Service-Key"))
                    .build());
        }
    }
}
